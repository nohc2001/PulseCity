...
void SkinMeshGameObject::InitRootBoneMatrixs()
{
	...
	if (BoneToWorldMatrixCB_Default.size() > 0) {
		// ªÁΩ« ¿Ã ƒ⁄µÂ¥¬ Ω««‡¿Ã µ«∏È æ»µ«¡ˆ æ ≥™? »Ï..
		// >> «œ¡ˆ∏∏ º≠πˆø°º≠ ¿⁄Ω≈¿« «√∑π¿ÃæÓ ø¿∫Í¡ß∆Æ µ•¿Ã≈Õ∏¶ ≈Î¬•∑Œ µŒπ¯ ∫∏≥ª±‚ ∂ßπÆø°..
		// >> ∞·±π µŒπ¯ µ«¥¬∞« æÓ¬ø ºˆ æ¯¥¬ ∞Õ¿∏∑Œ ∫∏¿Œ¥Ÿ. ¿ﬂ ∏±∏Æ¡Ó∏¶ «ÿ∫∏¿⁄.
		// >> ∞‘¿” ªÛ»≤ø°º≠ shape∞° πŸ≤ ºˆµµ ¿÷¿›¿Ω? §∑§∑..
...
{
	// æÀ∞Ì∏Æ¡Ú ∂ßπÆø° ∫¥∏Ò¿Ã ª˝∞Âæ˙¿Ω. ±◊∑°º≠ ∞Ì√∆¥Ÿ.
	// ¥Ÿ∏∏ 20∏Ì¿« Ω∫≈≤∏ﬁΩ¨ æ÷¥œ∏ﬁ¿Ãº« ∞¢ √‚∑¬«œ¥¬µ• 
	// «— «¡∑π¿”¿« 0.029124 ¿ª ¿‚æ∆∏‘¥¬∞« ¡ª Ω…«œ¥Ÿ. ±◊∑≥ 50∏Ì∏∏ ¿÷æÓµµ «¡∑π¿”¿∫ ∞≥ ≥∑æ∆¡˙∞≈ æ∆¥—∞°?
	// ∂ßπÆø° GPU √÷¿˚»≠ µµ √Ê∫–»˜ ∞Ì∑¡«ÿæﬂ «“ ∞Õ ∞∞¥Ÿ.
...
void SkinMeshGameObject::SetShape(Shape _shape)
{
	...
	if (model != nullptr) {
		...
		if (gd.isSupportRaytracing) {
			...
			for (int i = 0; i < model->mNumSkinMesh; ++i) {
				// ºˆ¡§«“ πˆ≈√Ω∫∏∏ ∏∏µÈæÓ rmeshø° ¿˙¿Â. √‚∑¬Ω√ LRSø°º≠ πˆ≈√Ω∫¥¬ ø©±‚ø°º≠, ¿Œµ¶Ω∫¥¬ ø¯∫ª∏ﬁΩ¨ø°º≠ ∞°¡Æø¬¥Ÿ.
				...
				//Compute Geometry ∫Ø«¸¿ª ¿ß«— UAV ª˝º∫
...
{
	// ComputeShader∞° ¿ÃπÃ Set µ«æ˙¥Ÿ¥¬ ∞°¡§«œø° «‘ºˆ∞° Ω««‡µ»¥Ÿ.
	Model* model = shape.GetModel();
	for (int i = 0; i < model->mNumSkinMesh; ++i) {
...
void SkinMeshGameObject::MoveChunck(const matrix& afterMat, const GameObjectIncludeChunks& beforeChunckInc, const GameObjectIncludeChunks& afterChunkInc) {
	...
	inter_up = 0;
	for (; ci.extra < chunckCount; afterChunkInc.Inc(ci)) {
		if (ci == inter_ci && inter_Count > 0) { // Å»ƒ°¥¬ ∫Œ∫–¿ª Alloc «œ¡ˆ æ ¥¬¥Ÿ.
			intersection.Inc(inter_ci);
			chunkAllocIndexs[ci.extra] = temp[inter_up];
			inter_up += 1;
#ifdef ChunckDEBUG 
...
void Portal::RecvSTC_SyncObj(char* data) {
	...
	// mesh/model ∆ƒΩÃ æ¯¿Ω ? ≈¨∂Û¿Ãæ∆Æ∞° ¿⁄√º Portal ∏ﬁΩ¨ ªÁøÎ
}
