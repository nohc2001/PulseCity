...
// ΩÃ≈©µ«¥¬ ∫Øºˆ¿« º≠πˆ¿Ã∏ß∞˙ ≈¨∂Û¿Ãæ∆Æ ¿Ã∏ß¿Ã ¥Ÿ∏• ∞ÊøÏ ø¨∞·¿ª ¿ß«ÿ ªÁøÎ.
...
void Game::SetLight()
{
	...
	UINT ncbElementBytes = ((sizeof(LightCB_DATA) + 255) & ~255); //256¿« πËºˆ
...
void Game::Init()
{
	...
	// πÃ∏Æ ∫£¿Ã≈©µ» ≈ÿΩ∫∆Æ¿« SDF ≈ÿΩ∫√ƒµÈ¿ª ∞°¡Æø¬¥Ÿ.
	gd.GetBakedSDFs(); // later

	// æ∆¿Ã≈€, NPCHPπŸ, Ray¿« 1¬˜ øÎ∑Æ¿ª º≥¡§«—¥Ÿ.
	DropedItems.reserve(4096);
	NpcHPBars.Init(1024);
	bulletRays.Init(1024);

	// ø©∑Ø ∏Æº“Ω∫µÈ¿ª √ ±‚»≠«œ±‚ ¿ß«ÿ ƒø∏«µÂ∏ÆΩ∫∆Æ∏¶ Reset«ÿ ∏Ì∑…¿ª ≥÷¿ª ¡ÿ∫Ò∏¶ «—¥Ÿ.
	gd.gpucmd.Reset();
	...
	// ≈¨∂Û¿Ãæ∆Æø°∏∏ ªÁøÎµ«¥¬ æ∆¡÷ ±‚∫ª¿˚¿Œ ø°º¬µÈ¿ª ∞°¡Æø¬¥Ÿ.
	{
...
void Game::InitDirLightGPURes() {
	UINT ncbElementBytes = ((sizeof(DirLightInfo) + 255) & ~255); //256¿« πËºˆ
...
void Game::Render() {
	...
	// 14. ∏µÁ π∞√º¥¬ Ω∫ƒ´¿Ãπ⁄Ω∫∫∏¥Ÿ æ’ø° øÕæﬂ «œ±‚ ∂ßπÆø° DepthStencil¿ª ≈¨∏ÆæÓ
	...
	if (CurrentRenderBatch) {
		...
		//18-1. ±◊∏≤¿⁄øÕ «‘≤≤ √‚∑¬«œ±‚ ¿ß«ÿ PBRShader Set, Root ∫Øºˆ¡ﬂ ±‚∫ª ∞Ì¡§µ«¥¬ ¡§∫∏∏¶ Set
		gd.gpucmd.SetShader(MyPBRShader1, ShaderType::RenderWithShadow);
		game.PresentShaderType = ShaderType::RenderWithShadow;
		gd.gpucmd->SetDescriptorHeaps(1, &gd.ShaderVisibleDescPool.pSVDescHeapForRender);
		{
	...
	// Diffuse ºŒ¿Ã¥ı∏¶ æ¯æ›¿Ω. ±◊∑°º≠ ¥Ÿ∏• ¥Î√§ πÊæ» « ø‰
	...
	//Command execution
	// 24-1. ª—ø∏∞‘ ∫∏¿Ã¥¬ π∞√ºµÈ¿ª ø¯∫ª∞˙ Depth∏¶ ∞∞¿Ã «’º∫«œ¥¬ ∞˙¡§
	// 24-2. GPU∑Œ ∫Ì∑Ø∏µ ≈ÿΩ∫√≥∏¶ ∞ËªÍ
	hResult = gd.gpucmd.Close();
	gd.gpucmd.Execute();
	gd.gpucmd.WaitGPUComplete();

	//Bluring + DepthConvolution(¡÷∏Òπﬁ¥¬ π∞√º¿« º±¿ª ¥ıøÌ ∫Ì∑Ø∏µ Ω√≈≤¥Ÿ.) (Compute Shader)
	...
	while (true) {
		...
		if (result == -1) {
			if (WSAGetLastError() == WSAEWOULDBLOCK) {
				// æ∆¡˜ ¿–¿ª ºˆ æ¯¥Ÿ∏È ¥Ÿ¿Ω ∑Á«¡∑Œ ≥—±‰¥Ÿ.
				break;
			}
			else {
				// ≥◊∆Æøˆ≈© ø¿∑˘∞° πﬂª˝µ«æ˙∞≈≥™ º≠πˆ∞° ¡◊¿∫ ªÛ»≤
				// TODO : º≠πˆøÕ¿« ø¨∞· ≤˜¿ª∂ß¿« √≥∏Æ
				break;
			}
		}
