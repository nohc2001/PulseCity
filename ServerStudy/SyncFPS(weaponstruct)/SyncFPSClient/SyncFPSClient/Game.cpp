...
void Game::SetLight()
{
	...
	UINT ncbElementBytes = ((sizeof(LightCB_DATA) + 255) & ~255); //256�� ���
	...
	for (int k = 0; k < 9; ++k) {
		...
		ncbElementBytes = ((sizeof(LightCB_DATA_withShadow) + 255) & ~255); //256??배�
...
namespace
{
	...
	auto GetParticleEffectRuntimes() -> std::vector<ParticleEffectRuntime>&
	{
		...
		effects.reserve(7);
		effects.push_back(ParticleEffectRuntime{ SkillEffectType::Mage_FireBall, &game.FirePool, &game.FireCS, &gFireSpriteSlot, "FireCS", Game::FIRE_COUNT, MakeParticleEmitter(vec4(-3.0f, 4.0f, 0.0f, 1.0f), vec4(0, 1, 0, 0), 1.0f, 1.0f, 0.0f, 0), true, true });
		effects.push_back(ParticleEffectRuntime{ SkillEffectType::Fire_Pillar, &game.FirePillarPool, &game.FirePillarCS, &gFirePillarSpriteSlot, "FirePillarCS", Game::FIRE_PILLAR_COUNT, MakeParticleEmitter(vec4(-3.0f, 0.5f, 0.0f, 1.0f), vec4(0, 1, 0, 0), 0.75f, 1.0f, 0.0f, 0), true, true });
		effects.push_back(ParticleEffectRuntime{ SkillEffectType::Fire_Ring, &game.FireRingPool, &game.FireRingCS, &gFireRingSpriteSlot, "FireRingCS", Game::FIRE_RING_COUNT, MakeParticleEmitter(vec4(-3.0f, 1.8f, 0.0f, 1.0f), vec4(0, 1, 0, 0), 1.0f, 1.0f, 0.0f, 0), true, true });
		effects.push_back(ParticleEffectRuntime{ SkillEffectType::Electric_Arc, &gElectricPool, &gElectricCS, &gElectricSpriteSlot, "ElectricArcCS", ELECTRIC_COUNT, MakeParticleEmitter(vec4(-1.6f, 1.85f, 0.0f, 1.0f), vec4(0, 1, 0, 0), 1.0f, 1.0f, 0.0f, 0), true, true });
		effects.push_back(ParticleEffectRuntime{ SkillEffectType::Electric_Burst, &gElectricBurstPool, &gElectricBurstCS, &gElectricBurstSpriteSlot, "ElectricBurstCS", ELECTRIC_BURST_COUNT, MakeParticleEmitter(vec4(0.0f, 1.4f, 0.0f, 1.0f), vec4(0, 1, 0, 0), 1.0f, 1.0f, 0.45f, 0), false, true });
		effects.push_back(ParticleEffectRuntime{ SkillEffectType::Ember_Shower, &gEmberShowerPool, &gEmberShowerCS, &gEmberShowerSpriteSlot, "EmberShowerCS", EMBER_SHOWER_COUNT, MakeParticleEmitter(vec4(0.0f, 5.6f, 0.0f, 1.0f), vec4(0, -1, 0, 0), 1.0f, 1.0f, 1.2f, 0), false, true });
		return effects;
	}
...
void Game::SpawnSkillEffect(SkillEffectType type, vec4 position, vec4 direction, UINT ownerId, float radius, float power, float duration)
{
	...
	case SkillEffectType::Aegis_ShieldAura:
		...
		break;
	case SkillEffectType::Frost_IceBlock:
	...
	if (type == SkillEffectType::Healer_HealAura) {
		...
		*effect->SpriteSlot = ParticleSpriteSlot::FireSecondary;
		...
		type == SkillEffectType::Aegis_ShieldCharge || type == SkillEffectType::Aegis_Barrier || type == SkillEffectType::Aegis_ShieldAura) {
		*effect->SpriteSlot = ParticleSpriteSlot::Electric;
	}
...
void Game::Init()
{
	...
	const int syncedGlobalMaterialStart = MaterialTable.size();
	{
		...
		PlayerModel->Retargeting_Humanoid(); // �޸ӳ��̵� ��Ÿ����
...
void Game::MoveZone(int zoneid) {
	...
	//	Map = new GameMap();
	//}
	// isAssetAddingInGlobal = false �� ���� ������ ��(��)�� �߰��Ǳ� �����Ѵ�. �� ��ȣ�� �ش�.
	//isAssetAddingInGlobal = false;
	...
	// ������ ���� ��� �۷ι� Asset���� SVDescHeap�� �÷����� �۾��� �Ѵ�.
	// ���� �� ũ�Ⱑ �ʹ� Ŀ���� �ȴٸ� Ŭ���̾�Ʈ���� ��� ������ ������ �����ؾ� �Ѵ�.
	...
	for (int i = 0; i < GlobalMaterialCount; ++i) {
		...
		if (isAssetAddingInGlobal == false) {
			...
			// MaterialTable to DescIndex > CBResource.resource.descindex.index�� ����.
			// CBResource.resource.descindex > MaterialTable ?? �̰� �ʿ��Ѱ�? 
			// >> �ʿ��� ��Ȳ�� �������� �ʴµ�? �׳� �̴�� �ص� ������ �ʳ�?
		}
	}
...
void Game::InitDirLightGPURes() {
	UINT ncbElementBytes = ((sizeof(DirLightInfo) + 255) & ~255); //256�� ���
...
void Game::Render() {
	...
	for (int i = 0; i < 9; ++i) {
		...
		if (nearzone->bReqireBakeLight_Raster) {
			...
			UINT ncbElementBytes = ((sizeof(ChunckLightData) * (nearzone->ChunckCountX * nearzone->ChunckCountY * nearzone->ChunckCountZ) + 255) & ~255); //256??배수
			...
			//MaterialStructuredBufferSRV�??�할?�하지 ?�는?? (같�? ?�리�?차�??�다.)
	...
	if (CurrentRenderBatch) {
		...
		//18-1. �׸��ڿ� �Բ� ����ϱ� ���� PBRShader Set, Root ������ �⺻ �����Ǵ� ������ Set
		gd.gpucmd.SetShader(MyPBRShader1, ShaderType::RenderWithShadow);
		game.PresentShaderType = ShaderType::RenderWithShadow;
		gd.gpucmd->SetDescriptorHeaps(1, &gd.ShaderVisibleDescPool.pSVDescHeapForRender);
		{
	...
	//Command execution
	// 24-1. �ѿ��� ���̴� ��ü���� ������ Depth�� ���� �ռ��ϴ� ����
	// 24-2. GPU�� ������ �ؽ�ó�� ���
	hResult = gd.gpucmd.Close();
	gd.gpucmd.Execute();
	gd.gpucmd.WaitGPUComplete();

	//Bluring + DepthConvolution(�ָ�޴� ��ü�� ���� ���� ������ ��Ų��.) (Compute Shader)
...
void Game::Render_ShadowPass()
{
	...
	// 0. ?�러?��???모두 ?�함?�면??Extent.z 방향??�?방향??OBB�?구성
	...
	for (int i = 0; i < 3; ++i) {
		...
		// 1-4. Dir Light ?�용 Upload Buffer??값을 ?�정?�다.
...
void UIInitInventoryWindow(DXUI* ui) {
	...
	pWindow->page_table.push_back(sample_page);
	{
		...
		*/
		int startx = 18;
		const wchar_t ItemTypes[4][32] = { L"���", L"�Һ�", L"��Ÿ", L"��ü" };
		for (int i = 0; i < 4; ++i) {
			rateloc = sample_page->location;
			rateloc.x += startx;
			rateloc.w -= 95;
		...
		//Item Slot ��. (12, 151, 75, 75) inc 80 7x7
		...
		for (int i = 0; i < inventory_slotCount; ++i) {
			...
			slotparam->addtionalParams[0] = rateloc.w; // �⺻ y ���۰�
			slotparam->addtionalParams[1] = inventory_slotCount / 7.0f; // ���� �� ����
			slotparam->addtionalPtr[1] = &pWindow->addtionalParams[1]; // �����̴� �� ������
			slotparam->addtionalPtr[2] = &slotShowRt; // Slot �� ���� �� �ִ� ����
			slotparam->addtionalParams_int[7] = i; // ���Թ�ȣ
...
void Game::UI_Init()
{
	...
	if (false)
	{
		...
		wcscpy_s(pEdit->text, 64, L"���𰡸� �Է��غ�����!");
		...
		pSliderX->setter = 0;
		pSliderX->mod = 'f';
		pSliderX->obj = new float(); // ���� Set�� �ܺ� ������ �ּҸ� �ִ´�. �ٸ� ������ new�� ���� �����.
		...
		pSliderY->setter = 0;
		pSliderY->mod = 'n';
		pSliderY->obj = new float(); // ���� Set�� �ܺ� ������ �ּҸ� �ִ´�. �ٸ� ������ new�� ���� �����.
	...
	//���� UI Init Code
	{
		...
		*/
		...
		pbtn_openInven->Set(0, 1, 6, L"�κ��丮");
		...
		pInventoryWindow->addtionalParams_int[0] = 0; // ���� ���õ� �������� Ÿ��. (0 : ��� / 1 : �Һ� / 2 : ��Ÿ / 3 : Ư��)
		pInventoryWindow->addtionalParams[1] = 0; // ���� �����ִ� ���� ���� �����̴��� ��
		pInventoryWindow->addtionalPtr[1] = new vec4(); // ������ ������ ���� �� �ִ� ������ ����.
