...
int World::NewPlayer(Player* obj, int clientIndex)
{
	...
	datacap = Sending_ChangeGameObjectMember(newindex, obj, GameObjectType::_Player, &((Player*)obj)->m_currentWeaponType, sizeof(int));
	gameworld.SendToAllClient_execept(datacap, clientIndex);
	pack_factory.push_data(datacap, tempbuffer.data);
	return newindex;
}
...
int World::Sending_PlayerFire(int objIndex) {
	int datacap = 6;
