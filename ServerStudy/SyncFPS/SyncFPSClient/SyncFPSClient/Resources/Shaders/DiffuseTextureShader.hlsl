...
float4 PSMain(VS_OUTPUT input) : SV_TARGET
{
    ...
    const float LightIntencity = 0.0f;
    const float LightRange = 0.0f;
    ...
    diffuse = saturate(dot(normalW, lightVector)); // dot normal & light direction 
    diffuseColor = float3(1.0f, 0.95f, 0.90f) * diffuseTexColor * diffuse;
    
    // Specular
    reflectDir = reflect(lightVector, normalW);
    //float3 halfVector = normalize(lightVector + eyeVector);
    specular = pow(saturate(dot(viewDir, reflectDir)), 32.0f); // dot half vector & normal
    specularColor = 0.25f * tempLightColor.rgb * specular;
