...
void DisplayMesh(FbxNode* pNode)
{
    ...
    if (view[ViewContent::Mesh_ControlPoint]) DisplayControlsPoints(lMesh);
	if (view[ViewContent::Mesh_Polygon]) DisplayPolygons(lMesh);
    DisplayMaterialMapping(lMesh);
    DisplayMaterial(lMesh);
	if(view[ViewContent::Mesh_Texture]) DisplayTexture(lMesh);
...
void DisplayPolygons(FbxMesh* pMesh)
{
    ...
    for (i = 0; i < lPolygonCount; i++)
    {
		...
		printf("        Polygon %d (vertex count : %d)\n", i, pMesh->GetPolygonSize(i));
        int l;
		...
		for (j = 0; j < lPolygonSize; j++)
		{
			...
			{
				FbxVector4 v4 = lControlPoints[lControlPointIndex];
				printf("            Coordinates: ctrlp[%d] (%g, %g, %g, %g)\n", lControlPointIndex, v4.mData[0], v4.mData[1], v4.mData[2], v4.mData[3]);
