...
{
	//RTV heap
	// [SwapChain RTV[SwapChainBufferCount]] [SubRenderTarget (for blur and raytracing)] [Defered Rendering GBuffer[GbufferCount]] [Light Probe Cube Map][0] ...
	// num descriptor : SwapChainBufferCount + GbufferCount + 1 + 6 * (LightProbeCount)
	...
	//DSV heap
	// [RT DSV] [DXR DSV] [DirectionLight CasCading Shadow DSV [4]] [pointLight CubeMap[6] Shadow DSV][0] [pointLight CubeMap[6] Shadow DSV][1] ...
	// num descriptor : 2 + max_DirLightCascadingShadowCount + 6 * (max_PointLightMaxCount)
...
BOOL ShaderVisibleDescriptorPool::AllocDescriptorTable(DescHandle* pOutDescriptor, UINT DescriptorCount)
{
	BOOL bResult = FALSE;
	if (m_AllocatedDescriptorCount + DescriptorCount > m_MaxDescriptorCount)
	{
	...
	pOutDescriptor->hcpu.ptr = m_cpuDescriptorHandle.ptr + m_AllocatedDescriptorCount * m_srvDescriptorSize;
	pOutDescriptor->hgpu.ptr = m_gpuDescriptorHandle.ptr + m_AllocatedDescriptorCount * m_srvDescriptorSize;
	m_AllocatedDescriptorCount += DescriptorCount;
	bResult = TRUE;
	return bResult;
}

BOOL ShaderVisibleDescriptorPool::ImmortalAllocDescriptorTable(DescHandle* pOutDescriptor, UINT DescriptorCount)
{
	BOOL bResult = FALSE;
	if (m_AllocatedDescriptorCount + DescriptorCount > m_MaxDescriptorCount)
	{
...
{
	return (m_pDescritorHeap->GetCPUDescriptorHandleForHeapStart().ptr <= hcpu.ptr) && (hcpu.ptr < m_pDescritorHeap->GetCPUDescriptorHandleForHeapStart().ptr + gd.CBVSRVUAVSize * m_MaxDescriptorCount);
}
...
void GameChunk::RenderChunkDbg() {
	gd.gpucmd.SetShader(game.MyOnlyColorShader);
	...
	auto Draw = [](vec4 start, vec4 dir) {
		matrix mat;
		float f = dir.len3;
		dir /= f;
		dir.w = 1;
		mat.LookAt(dir, vec4(1, 1, 1, 0));
		mat.right *= 10;
		mat.up *= 10;
		mat.look *= f;
		mat.pos = start;
		...
		BulletRay::mesh->Render(gd.gpucmd, 1);
	};
...
void Game::RenderTour()
{
	matrix idmat;
	idmat.Id();
	game.renderViewPort->UpdateFrustum();
	game.PresentChunkSeekDepth = 0;
	SameDepthChunkArr[0].clear();
	SameDepthChunkArr[1].clear();
	GameChunk* gc = GetChunkFromPos(game.renderViewPort->Camera_Pos);
			...
			IX_CHUNKFIND:
				...
				if (gci != chunck.end()) {
					...
					if (gc0->TourID != TourID) {
						if (game.renderViewPort->m_xmFrustumWorld.Intersects(gc->AABB)) {
					...
					if (game.renderViewPort->m_xmFrustumWorld.Intersects(ci.GetAABB())) {
						goto IX_CHUNKFIND;
					}
				}
			}
...
void Game::Init()
{
	...
	game.DefaultMaterial.ti.Diffuse = 0;
	game.DefaultMaterial.ti.Normal = 1;
	game.DefaultMaterial.ti.Ambient = 2;
	game.DefaultMaterial.ti.Metalic = 0;
	game.DefaultMaterial.ti.Roughness = 0;
	game.DefaultMaterial.SetDescTable();
	game.MaterialTable.push_back(game.DefaultMaterial);
	...
	for (int i = 0; i < Map->MapObjects.size(); ++i) {
		...
		if (goset.contains(Map->MapObjects[i]) == false) {
			goset.insert(Map->MapObjects[i]);
		}
	}
	int cnt = 0;
	TourID = rand();
	for (auto gc : chunck) {
		GameChunk* gcc = gc.second;
		for (int i = 0; i < gcc->Static_gameobjects.size; ++i) {
			if (gcc->Static_gameobjects.isnull(i)) continue;
			if (gcc->Static_gameobjects[i]->TourID != TourID) {
				cnt += 1;
				gcc->Static_gameobjects[i]->TourID = TourID;
			}
		}
	}
	...
	MyPointLight.CreatePointLight(PointLightCBData(XMFLOAT3(5, 5, 5), 1.0f, XMFLOAT3(1, 1, 1), 100));
	...
	/*TestCharMesh = new BumpMesh();
	TestCharMesh->MakeMeshFromWChar(gd.pDevice, gd.gpucmd, L'한', 100);*/
	...
	Model* BreakDance = new Model();
	BreakDance->LoadModelFile2("Resources/Model/Remy.model");
	BreakDance->Retargeting_Humanoid(); // 휴머노이드 리타겟팅

	HumanoidAnimation breakDanceAnim;
	breakDanceAnim.LoadHumanoidAnimation("Resources/Animation/BreakDance1990.Humanoid_animation");
	HumanoidAnimationTable.push_back(breakDanceAnim);

	constexpr float height = 40.0f;

	MyPlayer->SetShape(AW101);
	matrix mat = XMMatrixScaling(3, 3, 3);
	mat.pos.f3 = { 0.0f, 0, 0.0f };
	MyPlayer->worldMat = (mat);
	PushGameObject(MyPlayer);
	player = MyPlayer;
	...
	for (int i = 0; i < 20; ++i) {
		...
		Monster1->SetShape(AW101);
		Monster1->is_dead = false;
		Monster1->worldMat = XMMatrixScaling(3, 3, 3);
		Monster1->worldMat.pos.f3 = { (float)(rand() % 50), height, (float)(rand() % 50) };
		PushGameObject(Monster1);
	}

	/*DynamicGameObject* textobj = new DynamicGameObject();
	textobj->SetShape(TestCharMesh);
	textobj->worldMat.Id();
	textobj->worldMat.pos.y = -10;
	PushGameObject(textobj);*/

	SkinMeshGameObject* danceObj = new SkinMeshGameObject();
	danceObj->worldMat.pos = vec4(0, 5, -2, 1);
	danceObj->SetShape(BreakDance);
	danceObj->InitRootBoneMatrixs();
	danceObj->PushHumanoidAnimation(&game.HumanoidAnimationTable[0]);
	PushGameObject(danceObj);
	...
	myModelObject->worldMat.Id();
	myModelObject->worldMat.pos.f3 = { 0, -0.5, 1 };
...
void Game::Render() {
	...
	// 16. 테셀레이션 테스트 (16 - 컨트롤 포인트 실험)
	matrix Tr001;
	Tr001.pos.y = 2;
	Tr001.transpose();
	gd.gpucmd.SetShader(MyTesselationTestShader);
	gd.gpucmd->SetGraphicsRoot32BitConstants(0, 16, &view, 0);
	gd.gpucmd->SetGraphicsRoot32BitConstants(0, 4, &gd.viewportArr[0].Camera_Pos, 16);
	gd.gpucmd->SetGraphicsRoot32BitConstants(1, 16, &Tr001, 0);
	MytestTesselationMesh->Render(gd.gpucmd, 1);
	...
	//int cnt = 0;
	//game.TourID += 1;
	//for (pair<ChunkIndex, GameChunk*> gcI : chunck) {
	//	GameChunk* gc = gcI.second;
	//	if (true/*game.renderViewPort->m_xmFrustumWorld.Intersects(gc->AABB)*/) {
	//		for (int i = 0; i < gc->Static_gameobjects.size; ++i) {
	//			if (gc->Static_gameobjects.isnull(i)) continue;
	//			if (gc->Static_gameobjects[i]->TourID != TourID) {
	//				gc->Static_gameobjects[i]->Render();
	//				gc->Static_gameobjects[i]->TourID = TourID;
	//				cnt += 1;
	//			}
	//		}
	...
	//			if (gc->Dynamic_gameobjects.isnull(i)) continue;
	//			if (gc->Dynamic_gameobjects[i]->TourID != TourID) {
	//				gc->Dynamic_gameobjects[i]->Render();
	//				gc->Dynamic_gameobjects[i]->TourID = TourID;
	//			}
	//		}
	//	}
	...
	// 21. 렌더 마지막에 거울 렌더링 시작
	if(false)
	{
		// 21-1. 레스터 모드에서 반사되는 거울의 뒷면을 출력한다.
		...
		for (int i = 0; i < 16; ++i) {
			...
			mat.transpose();
			gd.gpucmd->SetGraphicsRoot32BitConstants(1, 16, &mat, 0);
			TestMirrorMesh->Render(gd.gpucmd, 1);
		}

		// 21-2. 거울의 부분에 스텐실을 1로 그린다.
		gd.gpucmd.SetShader(MyPBRShader1, ShaderType::RenderStencil);
		gd.gpucmd->OMSetStencilRef(1);
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 16, &view, 0);
		int materialIndex = 0;
		...
		gd.gpucmd->SetGraphicsRootDescriptorTable(3, material.TextureSRVTableIndex.hRender.hgpu);
		gd.gpucmd->SetGraphicsRootDescriptorTable(4, material.CB_Resource.descindex.hRender.hgpu);
		for (int i = 0; i < 16; ++i) {
		...
		// 21-3. 뎁스 버퍼를 클리어한다. (스텐실은 클리어하지 않는다.)
		gd.gpucmd->ClearDepthStencilView(d3dDsvCPUDescriptorHandle, D3D12_CLEAR_FLAG_DEPTH, 1.0f, 0, 0, NULL);
		
		// 21-4. 스텐실이 1인 부분에 렌더링을 하도록 한다.
		gd.gpucmd->OMSetStencilRef(1);
		...
		// 21-6. 거울의 표면과 반사행렬을 구한다.
		mirrorPlane = XMPlaneFromPointNormal(MirrorPoses[selection], vec4(1, 0, 0, 0)/*vec4(cosf(stackff), 0, sinf(stackff))*/);
		matrix reflect = XMMatrixReflect(mirrorPlane);
		//matrix sav = gd.viewportArr[0].ViewMatrix;

		// 21-7. 거울속 스카이박스를 그린다.
		MySkyBoxShader->RenderSkyBox(reflect, ShaderType::RenderInnerMirror);
		// 21-8. 거울속 스카이박스는 항상 거울속 물체들의 뒤에 있어야 하기 때문에 뎁스버퍼를 클리어해야 한다.
		gd.gpucmd->ClearDepthStencilView(d3dDsvCPUDescriptorHandle,
			D3D12_CLEAR_FLAG_DEPTH, 1.0f, 0, 0, NULL);
		// 21-9. PBR Shader를 RenderInnerMirror 모드로 Set한다.
		gd.gpucmd.SetShader(MyPBRShader1, ShaderType::RenderInnerMirror);
		game.PresentShaderType = ShaderType::RenderInnerMirror;
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 16, &view, 0);
	...
	// 23. UI 출력을 위해 DepthStencil을 Clear한다. (모든 UI는 이전에 그렸던 모든 것 위에 그려져야 하기 때문)
	gd.gpucmd->ClearDepthStencilView(d3dDsvCPUDescriptorHandle,
		D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, NULL);

	// 24. 플레이어의 일부가 가장 앞에 렌더링되어야 할때 렌더링 (ex> 총기)
	game.player->Render_AfterDepthClear();

	// 25. 글자출력을 위한 SDF 셰이더를 
	gd.gpucmd.SetShader(MyScreenCharactorShader, ShaderType::SDF);
	...
	//gd.gpucmd->SetGraphicsRoot32BitConstants(0, 7, MiniMapRootConstant, 0);
	...
	//gd.gpucmd->SetGraphicsRootDescriptorTable(1, miniMapHandle.hgpu);
	...
	//gd.gpucmd->SetGraphicsRoot32BitConstants(0, 7, MiniMapDotRootConstant, 0);
	...
	//gd.gpucmd->SetGraphicsRootDescriptorTable(1, miniMapPDotHandle.hgpu);
	//TextMesh->Render(gd.gpucmd, 1);
	//
	////player_direction Dot
	//vec4 plook = player->worldMat.look;
	//plook.y = 0;
	//plook.len3 = 1;
	...
	//gd.gpucmd->SetGraphicsRoot32BitConstants(0, 7, MiniMapDirDotRootConstant, 0);
	...
	//gd.gpucmd->SetGraphicsRootDescriptorTable(1, miniMapPDIRDotHandle.hgpu);
	//TextMesh->Render(gd.gpucmd, 1);
	//
	//for (int i = 0; i < 20; ++i) {
	//	Monster* mon = MonsterStartPtr[i];
	//	vec4 pos = mon->worldMat.pos;
	//	pos -= player->worldMat.pos;
	//	pos *= MiniMapZoomRate;
	...
	//	gd.gpucmd->SetGraphicsRootDescriptorTable(1, miniMapDotHandle.hgpu);
	//	TextMesh->Render(gd.gpucmd, 1);
	//}
		...
		for (int k = 0; k < SameDepthChunkArr[SDCAIndex].size(); ++k) {
			...
			IX_CHUNKFIND:
				...
				if (gci != chunck.end()) {
					GameChunk* gc0 = gci->second;
					if (gc0->TourID != TourID) {
						if (game.renderViewPort->OrthoFrustum.Intersects(gc->AABB)) {
							SameDepthChunkArr[SDCANextIndex].push_back(gc0);
						}
						gc0->TourID = TourID;
					}
					...
					if (game.renderViewPort->OrthoFrustum.Intersects(ci.GetAABB())) {
						goto IX_CHUNKFIND;
					}
				}
			}
...
GAMEOBJECTS_RENDER_END:*/
	...
	game.MyPointLight.RenderShadow();
...
{
	//CameraUpdate
	XMVECTOR vEye = player->worldMat.pos;
	...
	for (pair<ChunkIndex, GameChunk*> gcI : chunck) {
		GameChunk* gc = gcI.second;
		vec4 lendir = game.player->worldMat.pos;
		lendir - vec4(gc->AABB.Center);
		if (true/*lendir.fast_square_of_len3 < 10000*/) {
			for (int i = 0; i < gc->Dynamic_gameobjects.size; ++i) {
				if (gc->Dynamic_gameobjects.isnull(i)) continue;
				if (gc->Dynamic_gameobjects[i]->TourID != TourID) {
					gc->Dynamic_gameobjects[i]->Update(DeltaTime);
					gc->Dynamic_gameobjects[i]->TourID = TourID;
				}
			}
			...
			for (int i = 0; i < gc->SkinMesh_gameobjects.size; ++i) {
				if (gc->SkinMesh_gameobjects.isnull(i)) continue;
				if (gc->SkinMesh_gameobjects[i]->TourID != TourID) {
					gc->SkinMesh_gameobjects[i]->Update(DeltaTime);
					gc->SkinMesh_gameobjects[i]->TourID = TourID;
				}
			}
		}
	...
	/*for (int i = 0; i < m_gameObjects.size(); ++i) {
		DynamicGameObject* gbj1 = m_gameObjects[i];

		for (int j = i + 1; j < m_gameObjects.size(); ++j) {
			DynamicGameObject* gbj2 = m_gameObjects[j];
		...
		gbj1->LastQuerternion = gbj1->worldMat.getQ();
	...
	for (pair<ChunkIndex, GameChunk*> gcI : chunck) {
		...
		for (int i = 0; i < gc->Dynamic_gameobjects.size; ++i) {
			if (gc->Dynamic_gameobjects.isnull(i)) continue;
			if (gc->Dynamic_gameobjects[i]->TourID != TourID) {
				DynamicGameObject* gbj1 = gc->Dynamic_gameobjects[i];
				gbj1->Move(gbj1->tickLVelocity, gbj1->tickAVelocity);
				gbj1->tickLVelocity = XMVectorZero();
				gbj1->LastQuerternion = gbj1->worldMat.getQ();
				gbj1->tickAVelocity = 0;
				gbj1->TourID = TourID;
...
void Game::Release()
{
	...
	game.TourID += 1;
	vector<GameObject*> tempSav;
	for (pair<ChunkIndex, GameChunk*> gcI : chunck) {
		GameChunk* gc = gcI.second;
		for (int i = 0; i < gc->Static_gameobjects.size; ++i) {
			if (gc->Static_gameobjects.isnull(i)) continue;
			if (gc->Static_gameobjects[i]->TourID != TourID) {
				tempSav.push_back(gc->Static_gameobjects[i]);
				gc->Static_gameobjects[i]->TourID = TourID;
			}
		}
		...
		for (int i = 0; i < gc->Dynamic_gameobjects.size; ++i) {
			if (gc->Dynamic_gameobjects.isnull(i)) continue;
			if (gc->Dynamic_gameobjects[i]->TourID != TourID) {
				tempSav.push_back(gc->Dynamic_gameobjects[i]);
				gc->Dynamic_gameobjects[i]->TourID = TourID;
			}
		}
	}
	for (int i = 0; i < tempSav.size(); ++i) {
		delete tempSav[i];
	}
	tempSav.clear();
...
void Game::FireRaycast(GameObject* owner, vec4 rayStart, vec4 rayDirection, float rayDistance)
{
	vec4 rayFollow = rayStart;
	vec4 rayLDdir = chunck_divide_Width * rayDirection / rayDirection.len3;
	GameObject* closestHitObject = nullptr;

	float closestDistance = rayDistance;

	TourID += 1;
	GameChunk* prevGc = nullptr;

	for (float f = 0; f < rayDistance; f += chunck_divide_Width) {
		GameChunk* gc = GetChunkFromPos(rayFollow);
		if (gc == nullptr || gc == prevGc) {
			rayFollow += rayLDdir;
			continue;
		}
		else {
			for (int i = 0; i < gc->Static_gameobjects.size; ++i) {
				if (gc->Static_gameobjects.isnull(i)) continue;
				if (gc->Static_gameobjects[i]->TourID == TourID) continue;
				if (gc->Static_gameobjects[i]->tag[GameObject::GameObjectTag::Tag_Enable]) continue;
				float dist;
				if (gc->Static_gameobjects[i]->GetOBB().Intersects(rayStart, rayDirection, dist)) {
					if (dist < closestDistance) {
						closestDistance = dist;
						closestHitObject = gc->Static_gameobjects[i];
					}
				}
				gc->Static_gameobjects[i]->TourID = TourID;
			}
			...
			for (int i = 0; i < gc->Dynamic_gameobjects.size; ++i) {
				if (gc->Dynamic_gameobjects.isnull(i)) continue;
				if (gc->Dynamic_gameobjects[i]->TourID == TourID) continue;
				if (gc->Dynamic_gameobjects[i]->tag[GameObject::GameObjectTag::Tag_Enable]) continue;
				float dist;
				if (gc->Dynamic_gameobjects[i]->GetOBB().Intersects(rayStart, rayDirection, dist)) {
					if (dist < closestDistance) {
						closestDistance = dist;
						closestHitObject = gc->Dynamic_gameobjects[i];
					}
				}
				gc->Dynamic_gameobjects[i]->TourID = TourID;
			}
	...
	/*for (int i = 0; i < m_gameObjects.size(); ++i) {
		...
		if (obb.Intersects(rayStart, rayDirection, distance)) {
	...
	if (closestHitObject) {
		// hit calculation
		closestHitObject->OnRayHit(owner);
	}
}
...
void Game::RenderSDFText(const wchar_t* wstr, int length, vec4 Rect, float fontsiz, vec4 color, float* minD, float* maxD, float depth)
{
	...
	for (int i = 0; i < length; ++i) {
		...
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 1, &gd.ClientFrameWidth, 4);
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 1, &gd.ClientFrameHeight, 5);*/
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 14, &tConst, 0);
		MyScreenCharactorShader->SetTextureCommand(texture);
...
void Shader::CreateRootSignature()
{
	...
	rootParam[RootParamId::Const_Camera] = RootParam1::Const32s(GRegID('b', 0), 20, D3D12_SHADER_VISIBILITY_ALL);
	rootParam[RootParamId::Const_Transform] = RootParam1::Const32s(GRegID('b', 1), 16, D3D12_SHADER_VISIBILITY_VERTEX);
	rootParam[RootParamId::CBV_StaticLight] = RootParam1::CBV(GRegID('b', 2), D3D12_SHADER_VISIBILITY_PIXEL, D3D12_ROOT_DESCRIPTOR_FLAG_DATA_STATIC);
	...
	rootSigDesc1.NumParameters = RootParamId::Normal_RootParamCapacity;
...
void Shader::CreateRootSignature_withShadow()
{
	...
	DescTable0.DescTable(D3D12_SHADER_VISIBILITY_PIXEL);
	rootParam[RootParamId::SRVTable_ShadowMap] = DescTable0;
...
{
	gd.gpucmd->SetGraphicsRootDescriptorTable(3, shadowMapDesc.hgpu);
}
...
void OnlyColorShader::Add_RegisterShaderCommand(GPUCmd& cmd, ShaderType reg) {
	Shader::Add_RegisterShaderCommand(cmd, reg);
}
...
void DiffuseTextureShader::Add_RegisterShaderCommand(GPUCmd& cmd, ShaderType reg) {
	Shader::Add_RegisterShaderCommand(cmd, reg);
}
...
void ScreenCharactorShader::CreateRootSignature()
{
	...
	// rect(4) + screenWidht, screenHeight + depth
	rootParam[RootParamId::Const_BasicInfo] = RootParam1::Const32s(GRegID('b', 0), 7, D3D12_SHADER_VISIBILITY_VERTEX);
	RootParam1 DescTable0;
	DescTable0.PushDescRange(GRegID('t', 0), "SRV", 1, D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC);
	DescTable0.DescTable(D3D12_SHADER_VISIBILITY_PIXEL);
	rootParam[RootParamId::SRVTable_Texture] = DescTable0;
...
void ScreenCharactorShader::CreateRootSignature_SDF()
{
	...
	rootParam[RootParamId::Const_BasicInfo] = RootParam1::Const32s(GRegID('b', 0), 14, D3D12_SHADER_VISIBILITY_ALL);
	RootParam1 DescTable0;
	DescTable0.PushDescRange(GRegID('t', 0), "SRV", 1, D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC);
	DescTable0.DescTable(D3D12_SHADER_VISIBILITY_PIXEL);
	rootParam[RootParamId::SRVTable_Texture] = DescTable0;
...
void TesselationTestShader::Add_RegisterShaderCommand(GPUCmd& cmd, ShaderType reg)
{
	gd.gpucmd->SetGraphicsRootSignature(pRootSignature);
	gd.gpucmd->SetPipelineState(pPipelineState);
}
...
void GSBilboardShader::Add_RegisterShaderCommand(GPUCmd& cmd, ShaderType reg)
{
	cmd->SetGraphicsRootSignature(pRootSignature);
	cmd->SetPipelineState(pPipelineState);
}
...
void ParticleShader::Add_RegisterShaderCommand(GPUCmd& cmd, ShaderType reg)
{
	cmd->SetGraphicsRootSignature(pRootSignature);
	if (reg == ShaderType::StreamOut) {
		cmd->SetPipelineState(pPipelineState_StreamOutput);
	}
	else {
		cmd->SetPipelineState(pPipelineState);
	}
...
{
	//CreateVertexBuffer(gd.pDevice, gd.gpucmd, { 0, 0, 0 }, { 0, 1, 0 }, 5, vertexCount);
	...
	//create vertexBuffer
	VertexBufer = gd.CreateCommitedGPUBuffer(D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, D3D12_RESOURCE_DIMENSION_BUFFER, m_nVertices * sizeof(ParticleVertex0), 1);
	GPUResource VertexUploadBuffer = gd.CreateCommitedGPUBuffer(D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_DIMENSION_BUFFER, m_nVertices * sizeof(ParticleVertex0), 1);
	gd.UploadToCommitedGPUBuffer(&streamoutputVertexs[0], &VertexUploadBuffer, &VertexBufer, true);
	...
	StreamOutBuffer = gd.CreateCommitedGPUBuffer(D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, D3D12_RESOURCE_DIMENSION_BUFFER, m_nVertices * sizeof(ParticleVertex0), 1);
	//is require?
	StreamOutUploadBuffer = gd.CreateCommitedGPUBuffer(D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_DIMENSION_BUFFER, m_nVertices * sizeof(ParticleVertex0), 1);
	gd.UploadToCommitedGPUBuffer(&streamoutputVertexs[0], &StreamOutUploadBuffer, &StreamOutBuffer, true);
	StreamOutBuffer.AddResourceBarrierTransitoinToCommand(gd.gpucmd, D3D12_RESOURCE_STATE_STREAM_OUT);

	//why DrawBuffer exist??
	DrawBuffer = gd.CreateCommitedGPUBuffer(D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, D3D12_RESOURCE_DIMENSION_BUFFER, sizeof(m_nVertices * sizeof(ParticleVertex0)), 1);

	UINT64* pnBufferFilledSize = nullptr;
	BufferFilledSizeRes = gd.CreateCommitedGPUBuffer(D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_STREAM_OUT, D3D12_RESOURCE_DIMENSION_BUFFER, sizeof(UINT64), 1);
	//BufferFilledSizeRes.resource->Map(0, NULL, (void**) & pnBufferFilledSize);

	UINT64* pnUploadBufferFilledSize = nullptr;
	UploadBufferFilledSizeRes = gd.CreateCommitedGPUBuffer(D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_DIMENSION_BUFFER, sizeof(UINT64), 1);
	UploadBufferFilledSizeRes.resource->Map(0, NULL, (void**)&pnUploadBufferFilledSize);

	UINT64* pnReadBackBufferFilledSize = nullptr;
	ReadBackBufferFilledSizeRes = gd.CreateCommitedGPUBuffer(D3D12_HEAP_TYPE_READBACK, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_DIMENSION_BUFFER, sizeof(UINT64), 1);
...
void StreamOutputPoints::Render(bool isStreamOut)
{
	if (isStreamOut) {
		...
		BufferFilledSizeRes.AddResourceBarrierTransitoinToCommand(gd.gpucmd, D3D12_RESOURCE_STATE_COPY_DEST);
		gd.gpucmd->CopyResource(BufferFilledSizeRes.resource, UploadBufferFilledSizeRes.resource);
		BufferFilledSizeRes.AddResourceBarrierTransitoinToCommand(gd.gpucmd, D3D12_RESOURCE_STATE_STREAM_OUT);

		// Stream Out Render Particle
		gd.gpucmd.SetShader(game.MyParticleShader, ShaderType::StreamOut);
		gd.gpucmd->SOSetTargets(0, 1, &StreamOutBufferView);

		matrix view = gd.viewportArr[0].ViewMatrix;
		view.transpose();
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 16, &view, 0);
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 4, &gd.viewportArr[0].Camera_Pos, 16);
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 4, &worldMat.pos, 20);
		float DecreaseRate = 0.5f;
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 1, &DecreaseRate, 24);
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 1, &game.DeltaTime, 25);

		ui32 SlotNum = 0;
		gd.gpucmd->IASetVertexBuffers(SlotNum, 1, &VertexBuferView);
		gd.gpucmd->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_POINTLIST);
		ui32 VertexOffset = 0;
		gd.gpucmd->DrawInstanced(30/*m_nVertices*/, 1, VertexOffset, 0);

		// get how many vertex stream output
		/*ReadBackBufferFilledSizeRes.AddResourceBarrierTransitoinToCommand(gd.gpucmd, D3D12_RESOURCE_STATE_COPY_DEST);*/
		BufferFilledSizeRes.AddResourceBarrierTransitoinToCommand(gd.gpucmd, D3D12_RESOURCE_STATE_COPY_SOURCE);
		gd.gpucmd->CopyResource(ReadBackBufferFilledSizeRes.resource, BufferFilledSizeRes.resource);
		BufferFilledSizeRes.AddResourceBarrierTransitoinToCommand(gd.gpucmd, D3D12_RESOURCE_STATE_STREAM_OUT);
		...
		VertexBufer.AddResourceBarrierTransitoinToCommand(gd.gpucmd, D3D12_RESOURCE_STATE_COPY_DEST);
		StreamOutBuffer.AddResourceBarrierTransitoinToCommand(gd.gpucmd, D3D12_RESOURCE_STATE_COPY_SOURCE);
		gd.gpucmd->CopyResource(VertexBufer.resource, StreamOutBuffer.resource);
		VertexBufer.AddResourceBarrierTransitoinToCommand(gd.gpucmd, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
		StreamOutBuffer.AddResourceBarrierTransitoinToCommand(gd.gpucmd, D3D12_RESOURCE_STATE_STREAM_OUT);

		//VertexBuferView.SizeInBytes = sizeof(ParticleVertex0) * 30/*m_nVertices*/;
	}
	else {
		gd.gpucmd.SetShader(game.MyParticleShader, ShaderType::RenderNormal);
		ui32 SlotNum = 0;
		...
		DescHandle handle;
		gd.ShaderVisibleDescPool.DynamicAlloc(&handle, 1);
		gd.pDevice->CopyDescriptorsSimple(1, handle.hcpu, game.DefaultExplosionParticle.descindex.hCreation.hcpu, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

		gd.gpucmd->SetDescriptorHeaps(1, &gd.ShaderVisibleDescPool.pSVDescHeapForRender);
		gd.gpucmd->SetGraphicsRootDescriptorTable(1, handle.hgpu);

		gd.gpucmd->IASetVertexBuffers(SlotNum, 1, &VertexBuferView);
		gd.gpucmd->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_POINTLIST);
		ui32 VertexOffset = 0;
		gd.gpucmd->DrawInstanced(30, 1, VertexOffset, 0);
	}
}
...
void Mesh::ReadMeshFromFile_OBJ(const char* path, vec4 color, bool centering) {
	...
	// maybe.. State Error.
	gd.CreateDefaultHeap_VB(&temp_vertices[0], VertexBuffer, VertexUploadBuffer, VertexBufferView, m_nVertices, m_nStride);
	gd.CreateDefaultHeap_IB<sizeof(UINT)>(&TrianglePool[0], IndexBuffer, IndexUploadBuffer, IndexBufferView, TrianglePool.size() * 3);
	...
	_SubMeshIndexArr[1] = TrianglePool.size() * 3;
...
void Mesh::BatchRender(ID3D12GraphicsCommandList* pCommandList)
{
	for (int i = 0; i < subMeshNum; ++i) {
		...
		if (IndexNum > 0)
		{
			...
			if (InstanceData->InstanceSize < MinInstancingStartSize) {
				for (int k = 0; k < InstanceData->InstanceSize; ++k) {
					//transform
					pCommandList->SetGraphicsRoot32BitConstants(1, 16, &InstanceData->InstanceDataArr[0].worldMat, 0);
				...
				using PRID = PBRShader1::RootParamId;
				pCommandList->SetGraphicsRootDescriptorTable(PRID::SRVTable_Instancing_RenderInstance, InstanceData[i].InstancingSRVIndex.hRender.hgpu);
				//instancing..
				//set SRV..?
		...
		//			//draw
		//			ui32 VertexOffset = 0;
		//			pCommandList->DrawInstanced(VertexNum, singleinstanceNum, VertexOffset, 0);
		//		}
		//	}
		//	else {
		//		//instancing..
		//		//set SRV..?
		//		ui32 VertexOffset = 0;
		//		pCommandList->DrawInstanced(VertexNum, InstanceData->InstanceSize, VertexOffset, 0);
		//	}
		//	
		//}
	}
...
void Mesh::CreateGroundMesh(ID3D12GraphicsCommandList* pCommandList)
{
	...
	gd.CreateDefaultHeap_VB(&vertices[0], VertexBuffer, VertexUploadBuffer, VertexBufferView, nVertices, nStride);
	gd.CreateDefaultHeap_IB<sizeof(UINT)>(indices.data(), IndexBuffer, IndexUploadBuffer, IndexBufferView, indices.size());
	...
	int* _SubMeshIndexArr = new int[2];
	_SubMeshIndexArr[0] = 0;
	_SubMeshIndexArr[1] = indices.size() * 3;
	SubMeshIndexStart = _SubMeshIndexArr;
	subMeshNum = 1;
}
...
void Mesh::CreateWallMesh(float width, float height, float depth, vec4 color)
{
	...
	int* _SubMeshIndexArr = new int[2];
	_SubMeshIndexArr[0] = 0;
	_SubMeshIndexArr[1] = indices.size();
	SubMeshIndexStart = _SubMeshIndexArr;
	subMeshNum = 1;
}
...
void UVMesh::ReadMeshFromFile_OBJ(const char* path, vec4 color, bool centering)
{
	...
	gd.CreateDefaultHeap_VB(&temp_vertices[0], VertexBuffer, VertexUploadBuffer, VertexBufferView, m_nVertices, m_nStride);
	gd.CreateDefaultHeap_IB<sizeof(UINT)>(TrianglePool.data(), IndexBuffer, IndexUploadBuffer, IndexBufferView, TrianglePool.size() * 3);
...
void UVMesh::Render(ID3D12GraphicsCommandList* pCommandList, ui32 instanceNum, ui32 slotIndex)
{
	// question : what is StartSlot parameter??
	ui32 SlotNum = 0;
	pCommandList->IASetVertexBuffers(SlotNum, 1, &VertexBufferView);
...
void UVMesh::CreateTextRectMesh()
{
	...
	gd.CreateDefaultHeap_VB(vertices.data(), VertexBuffer, VertexUploadBuffer, VertexBufferView, nVertices, nStride);
	gd.CreateDefaultHeap_IB<sizeof(UINT)>(indices.data(), IndexBuffer, IndexUploadBuffer, IndexBufferView, indices.size());
...
BumpMesh* BumpMesh::MakeTerrainMeshFromHeightMap(const char* HeightMapTexFilename, float bumpScale, float Unit, int& XN, int& ZN, byte8** Heightmap, bool include_DXR)
{
	...
	*Heightmap = imageRaw;
	...
	for (int xi = 0; xi < Divide[0]; ++xi) {
		for (int zi = 0; zi < Divide[1]; ++zi) {
			...
			gd.CreateDefaultHeap_VB(pVertices, mesh->VertexBuffer, mesh->VertexUploadBuffer, mesh->VertexBufferView, nVertices, sizeof(Vertex));
			gd.CreateDefaultHeap_IB<sizeof(UINT)>(TrianglePool.data(), mesh->IndexBuffer, mesh->IndexUploadBuffer, mesh->IndexBufferView, TrianglePool.size() * 3);
...
void Model::LoadModelFile(string filename)
{
	...
	for (int i = 0; i < mNumMeshes; ++i) {
		...
		for (int k = 0; k < vertSiz; ++k) {
			...
			ifs.read((char*)&vertice[i][k].u, sizeof(float));
			ifs.read((char*)&vertice[i][k].v, sizeof(float));
	...
	for (int i = 0; i < nodeCount; ++i) {
		...
		if (Nodes[i].numMesh != 0) {
			ifs.read((char*)&Nodes[i].Meshes[0], sizeof(int) * Nodes[i].numMesh);
			int matCount = mMeshes[Nodes[i].Meshes[0]]->subMeshNum;
			Nodes[i].materialIndex = new int[matCount];
			for (int k = 0; k < matCount; ++k) {
				Nodes[i].materialIndex[k] = ((BumpMesh*)mMeshes[Nodes[i].Meshes[0]])->material_index;
			}
		}
	}
	//new2
...
{
	tag[GameObject::GameObjectTag::Tag_Enable] = false;
	if (transforms_innerModel) {
		delete[] transforms_innerModel;
	}
}
...
{
	if (godmod == false) {
		if (worldMat.pos.fast_square_of_len3 > 100000000) {
			worldMat.pos = vec4(0, 10, 0, 1);
			LVelocity = 0;
		}
	}
	...
	if (m_pKeyBuffer['W'] & 0xF0) {
		tickLVelocity += speed * worldMat.look * game.DeltaTime;
	}
	if (m_pKeyBuffer['S'] & 0xF0) {
		tickLVelocity -= speed * worldMat.look * game.DeltaTime;
	}
	if (m_pKeyBuffer['A'] & 0xF0) {
		tickLVelocity -= speed * worldMat.right * game.DeltaTime;
	}
...
void Player::Render(matrix parent)
{
	...
	if (game.bFirstPersonVision == false) {
		...
		transforms_innerModel[0] = XMMatrixRotationAxis(vec4(0, 0, 1, 0), 3.141952f) * model->Nodes[0].transform;
		transforms_innerModel[3] = XMMatrixRotationAxis(vec4(0, 1, 0, 0), StackT) * model->Nodes[3].transform;
		transforms_innerModel[2] = XMMatrixRotationAxis(vec4(1, 0, 0, 0), StackT) * model->Nodes[2].transform;
		model->Render(gd.gpucmd, worldMat * parent, this);
	}
}
...
void Player::Render_AfterDepthClear()
{
	...
	{
		gd.gpucmd.SetShader(game.MyDiffuseTextureShader);
	...
	if (ShootFlow >= ShootDelay) {
		if (evt.Message == WM_LBUTTONDOWN) {
			if (game.bFirstPersonVision) {
				...
				game.FireRaycast(this, shootmat.pos + shootmat.look, shootmat.look, 50.0f);
			}
			else {
				matrix shootmat = gd.viewportArr[0].ViewMatrix.RTInverse;
				game.FireRaycast(this, shootmat.pos + shootmat.look * 3, shootmat.look, 50.0f);
			}
			ShootFlow = 0;
			recoilFlow = 0;
		}
...
void Player::OnCollision(GameObject* other)
{
	...
	if (belowhit && belowDist < GetOBB().Extents.y + 0.4f) {
		LVelocity.y = 0;
		isGround = true;
	}
}

void Player::OnRayHit(GameObject* rayFrom) {
...
void Monster::Update(float deltaTime)
{
	if (is_dead == false) {
		...
		worldMat.pos.x += rand_direction.x * Speed * game.DeltaTime;
		worldMat.pos.z += rand_direction.z * Speed * game.DeltaTime;
	}
	else {
		Model* model = shape.GetModel();
		if (transforms_innerModel[0].pos.y <= -100) {
			is_dead = false;
			worldMat = XMMatrixScaling(3, 3, 3);
			worldMat.pos.f3 = { (float)(rand() % 50), 40, (float)(rand() % 50) };
			for (int i = 0; i < model->nodeCount; ++i) {
				transforms_innerModel[i] = model->Nodes[i].transform;
			}
		}
...
void Monster::OnRayHit(GameObject* rayFrom) {
	is_dead = true;
...
void TerrainObject::LoadTessTerrain(const wchar_t* filenae, float EdgeLen, int div, vec4 Center)
{
	...
	gd.gpucmd->ResourceBarrier(1, &barrier);

	CenterX = Center.x;
	CenterZ = Center.z;
	CenterY = Center.y;
	Material mat = LoadTerrainMaterial(
		L"Resources/GlobalTexture/TerrainMaterials/Ground015_2K-JPG_Color.jpg",
		L"Resources/GlobalTexture/TerrainMaterials/Ground015_2K-JPG_NormalDX.jpg",
		L"Resources/GlobalTexture/TerrainMaterials/Ground015_2K-JPG_AmbientOcclusion.jpg",
		L"Resources/GlobalTexture/TerrainMaterials/Ground015_2K-JPG_Roughness.jpg",
	...
	game.MaterialTable.push_back(mat);
	BaseMaterialIndex = game.MaterialTable.size();
}
...
void TerrainObject::Render()
{
	...
	view *= gd.viewportArr[0].ProjectMatrix;
	view.transpose();
	Material* BaseMaterial = &game.MaterialTable[BaseMaterialIndex];
	D3D12_GPU_DESCRIPTOR_HANDLE hGPU = BaseMaterial->TextureSRVTableIndex.hRender.hgpu;
	ShaderType shadertype = ShaderType::RenderWithShadow;

	if (isTesslation) shadertype = ShaderType::TessTerrain;
	gd.gpucmd.SetShader(game.MyPBRShader1, shadertype);
	gd.gpucmd->SetDescriptorHeaps(1, &gd.ShaderVisibleDescPool.pSVDescHeapForRender);
	gd.gpucmd->SetGraphicsRoot32BitConstants(0, 16, &view, 0);
	gd.gpucmd->SetGraphicsRoot32BitConstants(0, 4, &gd.viewportArr[0].Camera_Pos, 16);
	gd.gpucmd->SetGraphicsRootConstantBufferView(2, game.LightCB_withShadowResource.resource->GetGPUVirtualAddress());
	gd.gpucmd->SetGraphicsRootDescriptorTable(3, hGPU);
	gd.gpucmd->SetGraphicsRootDescriptorTable(4, BaseMaterial->CB_Resource.descindex.hRender.hgpu);
	gd.gpucmd->SetGraphicsRootDescriptorTable(5, game.MyDirLight.descindex.hRender.hgpu);
	...
	if (isTesslation) {
		...
		m.pos = pos;
		m.transpose();
		gd.gpucmd->SetGraphicsRootDescriptorTable(6, HeightMap.descindex.hRender.hgpu);
		gd.gpucmd->SetGraphicsRoot32BitConstants(1, 16, &m, 0);
		TerrainMeshs->Render(gd.gpucmd, 1);
	}
	else {
		for (int xi = 0; xi < XWid; ++xi) {
			for (int zi = 0; zi < ZWid; ++zi) {
				...
				if (gd.viewportArr[0].m_xmFrustumWorld.Intersects(obb)) {
					...
					mat.transpose();
					gd.gpucmd->SetGraphicsRoot32BitConstants(1, 16, &mat, 0);
					TerrainMeshs[xi * ZWid + zi].Render(gd.gpucmd, 1);
				}
			}
		}

		gd.gpucmd.SetShader(game.MyGrassShader);
		gd.gpucmd->SetDescriptorHeaps(1, &gd.ShaderVisibleDescPool.pSVDescHeapForRender);
		...
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 16, &proj, 0);
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 16, &view, 16);
		gd.gpucmd->SetGraphicsRoot32BitConstants(0, 4, &gd.viewportArr[0].Camera_Pos, 32);

		matrix world;
		world.pos.f3 = { CenterX , CenterY, CenterZ };
		world.transpose();
		gd.gpucmd->SetGraphicsRoot32BitConstants(1, 16, &world, 0);
		gd.gpucmd->SetGraphicsRootDescriptorTable(2, game.DefaultGrass.descindex.hRender.hgpu);
		GrassPointMesh->Render(gd.gpucmd, 1);
	}
}
...
{
	BoundingOrientedBox obb = other->GetOBB();
	XMFLOAT3 points[8];
