...
#define __tt(t, a) reinterpret_cast<t>(a)
...
#pragma endregion
	...
	__forceinline const bool operator==(const vec4& ref) const { return XMColorEqual(v4, ref); }
	__forceinline const bool operator!=(const vec4& ref) const { return !XMColorEqual(v4, ref); }
...
public:
	...
	straightLine2D() { xrate = 0; yrate = 0; inDot = vec2f(); }
	straightLine2D(const straightLine2D& ref) { xrate = ref.xrate; yrate = ref.yrate; inDot = ref.inDot; }
	straightLine2D(float xr, float yr, vec2f indot) { xrate = xr; yrate = yr; inDot = indot; }
	straightLine2D(vec2f p1, vec2f p2) { xrate = p2.x - p1.x; yrate = p2.y - p1.y; inDot = vec2f(p1); }
	...
	bool operator==(const straightLine2D& ref) {
...
BoundingBox OBBToAABB(const BoundingOrientedBox& obb)
{
	// OBB의 8개 코너를 얻는다
	...
	BoundingBox::CreateFromPoints(
		aabb,
		BoundingOrientedBox::CORNER_COUNT,
		corners,
		sizeof(XMFLOAT3)
	);
...
vec2f GetCrossPoint(straightLine2D sl1, straightLine2D sl2)
{
	vec2f cross;
...
float get_distance_PointAndLine_inRange(vec2f p, vec2f l1, vec2f l2) {
	straightLine2D line = straightLine2D(l1, l2);
	straightLine2D rline = straightLine2D(p, vec2f(p.x + line.yrate, p.y - line.xrate));
	vec2f cross = GetCrossPoint(line, rline);
	bool bx, by;
	bx = (((l1.x <= cross.x && cross.x <= l2.x) || (l2.x <= cross.x && cross.x <= l1.x)) || line.xrate == 0);
	by = (((l1.y <= cross.y && cross.y <= l2.y) || (l2.y <= cross.y && cross.y <= l1.y)) || line.yrate == 0);
	if (bx || by) {
		return get_distance(p, cross);
	}
	else return -1.0f;
}
