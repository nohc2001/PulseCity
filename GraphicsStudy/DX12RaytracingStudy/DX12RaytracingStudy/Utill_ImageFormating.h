...
namespace imgform {
    ...
    public:
        ...
        PixelImageObject(int wid, int hei, void* d) :
        ...
        void rawDataToBMP(const std::string& filename, DXGI_FORMAT format = DXGI_FORMAT_R8G8B8A8_UNORM) {
            if (format == DXGI_FORMAT_R8G8B8A8_UNORM) {
                ...
                for (int y = height - 1; y >= 0; y--) {
                    for (int x = 0; x < width; x++) {
                        file.put(data[(y * width + x)].b);
                        file.put(data[(y * width + x)].g);
                        file.put(data[(y * width + x)].r);
                        file.put(data[(y * width + x)].a);
                    }
                ...
                for (int y = height - 1; y >= 0; y--) {
                    for (int x = 0; x < width; x++) {
                        BYTE c = *reinterpret_cast<BYTE*>(&ptr[(y * width + x)]) /*+ 128*/;
        ...
        void rawDataToBMP(const std::wstring& filename) {
            std::ofstream file(filename, std::ios::binary);
            if (!file) {
                std::cerr << "Error opening file for writing." << std::endl;
                return;
