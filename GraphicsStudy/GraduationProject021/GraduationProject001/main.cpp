...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdParam, int nCmdShow) {
	...
	clientsocket->Bind(Endpoint::Any);
	clientsocket->Connect(Endpoint("127.0.0.1", 5959));
...
void Game::Update()
{
	...
	if (key != 0) {
		clientsocket->Send(&key, 1);
	}

	int result = clientsocket->Receive();
	if (result > 0) {
		float x, z;
		memcpy(&x, clientsocket->m_receiveBuffer, sizeof(float));
		memcpy(&z, clientsocket->m_receiveBuffer + sizeof(float), sizeof(float));

		//moving object
		MyObjectWorldMat.r[3].m128_f32[0] = x;
		MyObjectWorldMat.r[3].m128_f32[2] = z;
	}
