...
public:
	...
	static std::vector<Mesh> LoadFromGLTF(ID3D12GraphicsCommandList* pCommandList, const std::string& filename);
	void CreateGPUBuffers(ID3D12GraphicsCommandList* pCommandList, const std::vector<Vertex>& vertices, const std::vector<UINT>& indices);
};
