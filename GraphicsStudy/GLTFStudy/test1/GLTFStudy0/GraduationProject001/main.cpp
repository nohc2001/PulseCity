...
void GlobalDevice::UploadToCommitedGPUBuffer(ID3D12GraphicsCommandList* commandList, void* ptr, GPUResource* uploadBuffer, GPUResource* copydestBuffer, bool StateReturning)
{
	...
	uploadBuffer->resource->Unmap(0, NULL);
...
void Game::Init()
{
	...
	MyMeshes = Mesh::LoadFromGLTF(gd.pCommandList, "Resources/Mesh/robot/scene.gltf");
...
void Mesh::CreateGPUBuffers(ID3D12GraphicsCommandList* pCommandList, const std::vector<Vertex>& vertices, const std::vector<UINT>& indices)
{
	VertexNum = vertices.size();
	if (VertexNum > 0) {
		UINT vertexBufferSize = sizeof(Vertex) * VertexNum;

		VertexBuffer = gd.CreateCommitedGPUBuffer(pCommandList, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, D3D12_RESOURCE_DIMENSION_BUFFER, vertexBufferSize, 1);
		VertexUploadBuffer = gd.CreateCommitedGPUBuffer(pCommandList, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_DIMENSION_BUFFER, vertexBufferSize, 1);
		gd.UploadToCommitedGPUBuffer(pCommandList, const_cast<Vertex*>(vertices.data()), &VertexUploadBuffer, &VertexBuffer, true);
		VertexBuffer.AddResourceBarrierTransitoinToCommand(pCommandList, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
	...
	if (IndexNum > 0) {
		UINT indexBufferSize = sizeof(UINT) * IndexNum;

		IndexBuffer = gd.CreateCommitedGPUBuffer(pCommandList, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_INDEX_BUFFER, D3D12_RESOURCE_DIMENSION_BUFFER, indexBufferSize, 1);
		IndexUploadBuffer = gd.CreateCommitedGPUBuffer(pCommandList, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_DIMENSION_BUFFER, indexBufferSize, 1);
		gd.UploadToCommitedGPUBuffer(pCommandList, const_cast<UINT*>(indices.data()), &IndexUploadBuffer, &IndexBuffer, true);
		IndexBuffer.AddResourceBarrierTransitoinToCommand(pCommandList, D3D12_RESOURCE_STATE_INDEX_BUFFER);
