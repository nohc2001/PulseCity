...
{
	int zid = gameworld.clients[this->clientIndex].zoneId;
	Zone& zone = gameworld.zones[zid];

	/*if (gameworld.lowHit()) {
		cout << "[Player " << clientIndex << "] pos=("
			<< m_worldMatrix.pos.x << ", "
			<< m_worldMatrix.pos.y << ", "
			<< m_worldMatrix.pos.z << ") objIdx="
			<< gameworld.clients[clientIndex].objindex << endl;
	...
	if (HealSkillCooldownFlow >= 0.0f) {
		...
		int datacap = gameworld.Sending_ChangeGameObjectMember(gameworld.currentIndex, this, GameObjectType::_Player, &HealSkillCooldownFlow, sizeof(HealSkillCooldownFlow));
	...
	if (InputBuffer[InputID::KeyboardQ] == true) {
		...
		if (HealSkillCooldownFlow <= 0.0f && HeatGauge > 0.0f && HP < MaxHP) {
			...
			int datacap = gameworld.Sending_ChangeGameObjectMember(gameworld.currentIndex, this, GameObjectType::_Player, &HP, sizeof(HP));
			gameworld.clients[clientIndex].socket.Send(gameworld.tempbuffer.data, datacap);

			datacap = gameworld.Sending_ChangeGameObjectMember(gameworld.currentIndex, this, GameObjectType::_Player, &HeatGauge, sizeof(HeatGauge));
			gameworld.clients[clientIndex].socket.Send(gameworld.tempbuffer.data, datacap);
		}
	}
	...
	if (InputBuffer[InputID::MouseLbutton] == true) {
		if (ShootFlow >= ShootDelay && bullets > 0 && HeatGauge <= MaxHeatGauge) {
			...
			int datacap = gameworld.Sending_ChangeGameObjectMember(gameworld.currentIndex, this, GameObjectType::_Player, &bullets, 4);
			gameworld.clients[clientIndex].socket.Send(gameworld.tempbuffer.data, datacap);

			datacap = gameworld.Sending_ChangeGameObjectMember(gameworld.currentIndex, this, GameObjectType::_Player, &HeatGauge, 4);
			gameworld.clients[clientIndex].socket.Send(gameworld.tempbuffer.data, datacap);
			//cout << bullets << " bullets remain." << endl;

			matrix shootmat = ViewMatrix.RTInverse;
			zone.FireRaycast((GameObject*)this, shootmat.pos + shootmat.look * 3, shootmat.look, 50.0f);
			ShootFlow = 0;
			recoilFlow = 0;
	...
	for (int i = 0; i < zone.DropedItems.size; ++i) {
		...
		if (obb.Contains(p)) {
			...
			for (int k = 0; k < Inventory.size(); ++k) {
				...
				if (Inventory[k].id == zone.DropedItems[i].itemDrop.id)
				{
					Inventory[k].ItemCount += zone.DropedItems[i].itemDrop.ItemCount;
					isexist = true;
					beat = true;
					firstBlackIndex = k;
					break;
			...
			if (isexist == false && firstBlackIndex != -1) {
				Inventory[firstBlackIndex] = zone.DropedItems[i].itemDrop;
				beat = true;
			}
...
{
	int zid = gameworld.clients[clientIndex].zoneId;
	Zone& myZone = gameworld.zones[zid];

	HP -= damage;

	int datacap = gameworld.Sending_ChangeGameObjectMember(gameworld.currentIndex, this, GameObjectType::_Player, &HP, 4);
	gameworld.clients[clientIndex].socket.Send(gameworld.tempbuffer.data, datacap);
	...
	if (HP <= 0) {
		...
		DeathCount += 1;
		int datacap = gameworld.Sending_ChangeGameObjectMember(gameworld.currentIndex, this, GameObjectType::_Player, &DeathCount, 4);
		gameworld.clients[clientIndex].socket.Send(gameworld.tempbuffer.data, datacap);
...
void Player::Respawn() {
	...
	int datacap = gameworld.Sending_ChangeGameObjectMember(gameworld.currentIndex, this, GameObjectType::_Player, &isExist, 1);
	gameworld.SendToAllClient(datacap);

	datacap = gameworld.Sending_ChangeGameObjectMember(gameworld.currentIndex, this, GameObjectType::_Player, &HP, 4);
	gameworld.SendToAllClient(datacap);
}