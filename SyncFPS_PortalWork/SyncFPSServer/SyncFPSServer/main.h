...
union SendingType {
	enum {
		...
		ChangeServer = 12, // [st] [ip(16)] [port(4)] [spawnpos(12)]
		ChangeMap = 13, // [st] [mapid(4)]
	};
...
struct World {
	...
	*/
	...
	void InitZone(int zoneId);  // 각 존별 초기화

	// 특정 존에 있는 클라이언트에게만 전송
	void SendToZone(int zoneId, int datacap) {
	...
	void SendToZone_except(int zoneId, int datacap, int except) {
		for (int i = 0; i < clients.size; ++i) {
			if (clients.isnull(i)) continue;
	...
	// 존 이동
	void MovePlayerToZone(int clientIndex, int toZoneId, vec4 spawnPos);
	void SendZoneDataToClient(int clientIndex, int zoneId);

	/*
	포탈 스폰 함수
	*/
	void SpawnPortal(int Zoneid);

	/*
	플레이어가 포탈과 충돌했는지 검사하는 함수
	*/
	...
	* 반환값은 추가된 오브젝트가 게임오브젝트 배열에 몆번째 위치에 있는지에 대한 인덱스.
	*/
	int NewPlayer(Player* obj, int clientIndex,int zoneid);
	...
	*/
	...
	/*
	* 설명 : 플레이어가 포탈에 접촉했을시 기존 서버의 연결을 끊고, 목적지 서버에 연결
	*/
	void SendPlayerDatatoOtherServer(int clientIndex, const char* ip, int portnum, int dstzoneid, const vec4& spawnPos);

	/*
	* 설명 : 플레이어가 포탈에 접촉했을시 새로운 위치에 스폰
	*/
	void SendPlayerDatatoOtherZone(int clientIndex, int dstzoneId, const vec4& spawnPos);
	...
	* 구성된 패킷의 사이즈를 반환
	*/
	int Sending_NewRay(vec4 rayStart, vec4 rayDirection, float rayDistance);
	...
	*/
	void SendingAllObjectForNewClient(int zoneId);
	...
	Zone* GetClientZone(int clientIndex) {
		...
		return &zones[clients[clientIndex].zoneId];  // & 붙여서 주소 반환
	}

	Zone* GetZone(int zoneId) {
		if (zoneId < 0 || zoneId >= (int)zones.size()) return nullptr;
		return &zones[zoneId];  // & 붙여서 주소 반환
	}
};
