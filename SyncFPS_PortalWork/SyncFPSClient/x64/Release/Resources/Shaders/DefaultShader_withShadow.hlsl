...
struct DirectionLight
{
    ...
    float3 gLightColor; // Light_Color
};
...
Texture2D LightShdowMap : register(t0);
...
VS_OUTPUT VSMain(VS_INPUT input)
{
    ...
    output.cameraPosW = Camera_Position;//mul(Camera_Position, gWorld);
    return (output);
}
...
{   
    ...
    float depth = LightShdowMap.Sample(StaticSampler, vShadowTexCoord).r;
    ...
    // Diffuse
    float3 LightDirection = dirLight.gLightDirection;
    float3 lightVector = -LightDirection;
    lightVector = normalize(lightVector);
    float diffuse = saturate(dot(normalW, lightVector)); // dot normal & light direction 
    float3 diffuseColor = dirLight.gLightColor * diffuseTexColor * diffuse;
    
    // Specular
    float3 reflectDir = reflect(lightVector, normalW);
    float specular = pow(saturate(dot(viewDir, reflectDir)), 32.0f); // dot half vector & normal
    float3 specularColor = 0.25f * dirLight.gLightColor * specular;
