...
union GameObjectType {
	static constexpr int ObjectTypeCount = 4;
...
void Game::Init()
{
	...
	//임시로 포탈을 표시할 벽 매쉬 생성
	BumpMesh* MyPortalMesh = new BumpMesh();
	MyPortalMesh->CreateWallMesh(2.0f, 4.0f, 2.0f, { 0, 0, 1, 1 });
...
void Game::Render() {
	...
	for (int i = 0; i < m_gameObjects.size(); ++i) {
		...
		if (m_gameObjects[i]->m_pMesh == nullptr && m_gameObjects[i]->pModel == nullptr) continue;
		if (m_gameObjects[i]->m_pShader == nullptr) continue;
...
void Game::Render_ShadowPass()
{
	...
	if (game.isPreparedGo == false) return;
	if (game.isPrepared == false) return;
...
void Game::Update()
{
	...
	if (pendingChangeServer)
	{
		pendingChangeServer = false;
		OutputDebugStringA("[ChangeServer] Executing server change...\n");

		// 1) 기존 소켓 닫기
		ClientSocket->Close();

		pack_factory.Clear();
		memset(recvSavBuff, 0, sizeof(recvSavBuff));
		recvIsCutting = false;
		recvSavBuffup = 0;
		recvCuttingType = 't';
		recvIsPackReading = false;

		// 2) 기존 게임 오브젝트 정리
		for (int i = 0; i < m_gameObjects.size(); ++i) {
			if (m_gameObjects[i] != nullptr) {
				void* vptr = *(void**)m_gameObjects[i];
				if (vptr == GameObjectType::vptr[GameObjectType::_Player]) {
					Player* pp = (Player*)m_gameObjects[i];
					pp->Inventory.~vector();
					pp->gunBarrelNodeIndices.~vector();
				}
				free(m_gameObjects[i]);
		...
		bulletRays.Init(32);
		DropedItems.clear();
		DeltaMousePos = vec4(0, 0, 0, 0);

		// 4) 새 소켓 생성 및 연결
		delete ClientSocket;
		ClientSocket = new Socket(SocketType::Tcp);
		ClientSocket->SetReceiveBuffer(new twoPage, sizeof(twoPage));

		string addr = string(pendingServerIp) + ":" + to_string(pendingServerPort);
		OutputDebugStringA(("[ChangeServer] Connecting to " + addr + "\n").c_str());

		ClientSocket->Connect(Endpoint(pendingServerIp, pendingServerPort));
		currentzoneId = pendingdstzoneid;
		ClientSocket->SetNonblocking();

		OutputDebugStringA("[ChangeServer] Connected!\n");
		return;  // 이 프레임은 여기서 끝
	}
	...
	if (playerGameObjectIndex >= 0 && playerGameObjectIndex < m_gameObjects.size()) {
		...
		if (player != nullptr) {p->ClientUpdate(DeltaTime);}
...
int Game::Receiving(char* ptr, int totallen)
{
	...
	if (recvIsCutting) {
		...
		if (totallen + recvSavBuffup > sizeof(twoPage)) readlen = sizeof(twoPage) - recvSavBuffup;
		memcpy(&recvSavBuff[recvSavBuffup], ptr, readlen);
		recvIsCutting = false;
		int roffset = Receiving(recvSavBuff, sizeof(twoPage));
		offset = roffset - recvSavBuffup;
		recvSavBuffup = 0;
		return offset;
	}
	...
	if (totallen < offset + typedata_size) {
		recvIsCutting = true;
		recvCuttingType = 't';
		memcpy(recvSavBuff, ptr, totallen);
		recvSavBuffup = totallen;
		offset = totallen;
		return offset;
	}
	...
	{
		ui32 newobjindex = *(int*)&ptr[offset];
		offset += 4;
		GameObjectType gtype = *(GameObjectType*)&ptr[offset];
		if (gtype.id < 0 || gtype.id >= GameObjectType::ObjectTypeCount) {
			OutputDebugStringA("[ERROR] Invalid GameObjectType received\n");
			return totallen; // or force disconnect
		}
		...
		//just spawn check
		if (gtype == GameObjectType::_Portal)
		{
			OutputDebugStringA("[Portal] Spawned\n");
		}
		...
		*(void**)go = GameObjectType::vptr[gtype];
		...
		if (gtype == GameObjectType::_Player) {
			Player* pp = (Player*)go;
			new (&pp->Inventory) vector<ItemStack>();
			new (&pp->gunBarrelNodeIndices) vector<int>();
		}
	...
	case ServerInfoType::SetMeshInGameObject:
	{
		...
		else if (m_gameObjects[objindex]->m_pMesh == Shape::StrToShapeMap["Portal"].GetMesh()) {
			m_gameObjects[objindex]->MaterialIndex = 3;
		}
	}
	break;
	case ServerInfoType::AllocPlayerIndexes:
	{
		...
		char dbg[128];
		snprintf(dbg, sizeof(dbg), "[AllocPlayer] ci=%d oi=%d objsSize=%d\n",
			clientindex, objindex, (int)m_gameObjects.size());
		OutputDebugStringA(dbg);
	...
	case ServerInfoType::ItemDropRemove:
	{
		...
		if (dindex < 0 || dindex >= game.DropedItems.size()) {
			// 이전 서버 패킷이거나 이미 제거된 아이템
			return offset;
		}
	...
	break;
	case ServerInfoType::ChangeServer:
	{
		constexpr int payloadSize = 16 + 4 + 4 + 12; // ip + port + dstzoneid + spawn(xyz)

		// payload가 다 안 왔으면 cut 처리
		if (totallen < offset + payloadSize) {
			recvIsCutting = true;
			recvCuttingType = 'd';
			memcpy(recvSavBuff, ptr, totallen);
			recvSavBuffup = totallen;
		...
		char ipbuf[16] = {};
		memcpy(ipbuf, &ptr[offset], 16);
		ipbuf[15] = '\0'; // null 보장
		offset += 16;

		int port = *(int*)&ptr[offset];
		offset += 4;

		int dstzoneid = *(int*)&ptr[offset];
		offset += 4;

		XMFLOAT3 spawn3 = *(XMFLOAT3*)&ptr[offset];
		offset += 12;

		// 포트 sanity check
		if (port <= 0 || port > 65535) {
			OutputDebugStringA("[ChangeServer][ERROR] invalid port\n");
			return offset;
		}

		// 여기서 즉시 connect 하지 말고, "pending"으로 저장만 해
		// (메인 루프에서 상태 리셋 + 소켓 재생성 + connect)
		game.pendingChangeServer = true;
		game.isPrepared = false;
		game.isPreparedGo = false;
		game.player = nullptr;
		strncpy_s(game.pendingServerIp, ipbuf, 15);
		game.pendingServerPort = port;
		game.pendingdstzoneid = dstzoneid;
		game.pendingSpawnPos = vec4(spawn3.x, spawn3.y, spawn3.z, 1.0f);

		OutputDebugStringA("[ChangeServer] pending set\n");
		return offset;
	}
	case ServerInfoType::ChangeMap:
	{
		constexpr int payloadSize = 4 + 12; // dstzoneid + spawn(xyz)
		...
		// 기존 오브젝트 정리 (플레이어 제외)
		//for (int i = 0; i < m_gameObjects.size(); ++i) {
		//	//if (m_gameObjects[i] == player) continue;
		//	free(m_gameObjects[i]);
		//	m_gameObjects[i] = nullptr;
		//}
		m_gameObjects.clear();
		...
		NpcHPBars.Arr = nullptr;
		NpcHPBars.size = 0;
		NpcHPBars.endof_maxFlagarr = 0;
		NpcHPBars.Init(32);
		
		bulletRays.Arr = nullptr;
		bulletRays.size = 0;
		bulletRays.endof_maxFlagarr = 0;
		bulletRays.Init(32);      
		...
		player = nullptr;
		playerGameObjectIndex = -1;
		currentzoneId = dstzoneid;
		pendingSpawnPos = vec4(spawn3.x, spawn3.y, spawn3.z, 1.0f);

		isPrepared = false;
		isPreparedGo = false;
		preparedFlow = 0.0f;

		OutputDebugStringA("[ChangeMap] Zone changed\n");
		return offset;
	}
	}
	return offset;
