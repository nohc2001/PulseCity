...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdParam, int nCmdShow)
{
	...
	int desiredWidth = gd.EnableFullScreenMode_Resolusions[resolutionLevel].width;
	int desiredHeight = gd.EnableFullScreenMode_Resolusions[resolutionLevel].height;

	// 클라이언트 영역을 정확히 원하는 크기로 만들기
	...
	// 윈도우 생성 후 실제 클라이언트 크기 가져오기
	RECT clientRect;
	GetClientRect(hWnd, &clientRect);
	gd.ClientFrameWidth = clientRect.right - clientRect.left;
	gd.ClientFrameHeight = clientRect.bottom - clientRect.top;
	...
	char debugMsg[256];
	sprintf_s(debugMsg, "ClientFrameWidth: %d, ClientFrameHeight: %d\n",
		gd.ClientFrameWidth, gd.ClientFrameHeight);
	OutputDebugStringA(debugMsg);

	// 이제 gd.Init()과 game.Init()에서 올바른 크기 사용
	gd.Init();
	game.Init();
	...
	ClientSocket->Connect(Endpoint("127.0.0.1", 1001));
