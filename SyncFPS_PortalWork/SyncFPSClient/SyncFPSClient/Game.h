...
public:
	...
	//
	char recvSavBuff[sizeof(twoPage)] = {};
	int  recvSavBuffup = 0;
	char recvCuttingType = 't';
	bool recvIsCutting = false;
	bool recvIsPackReading = false;

	// 현재 접속한 존 아이디
	int currentzoneId = 0;

	//서버를 바꿀때 클라이언트의 게임 정보를 저장할 변수들 pending = 보류된
	bool pendingChangeServer = false;
	char pendingServerIp[16] = {};
	int pendingServerPort = 0;
	int pendingdstzoneid = 0;
	vec4 pendingSpawnPos = vec4(0, 0, 0, 1);

	// 서버 재접속을 위해 게임 상태를 리셋하는 함수
